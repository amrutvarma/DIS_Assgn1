        ...
        private static string RemoveVowels(string s)
        {
            try
            {
                ...
                {//Checking Vowels in string 's'
                    ...
                    {
                        final_string += "";// Vowels present in the string will be skipped
                    }
                    else
                    {
                        final_string += s[i];// If vowels are not present characters in string are concantenated.
                    }
                }
                return final_string;
            }
       ...
       */
        private static bool ArrayStringsAreEqual(string[] bulls_string1, string[] bulls_string2)
        {
            try
            {
                ...
                {
                    s1 = s1 + bulls_string1[i]; //concatenating all characters present in each index.
                }
                ...
                if (s1 == s2) //checks 's1','s2 are equivalent'
                {
                    return true;//It return 'true' if 's1','s2' they are equal
                }

                else
                {
                    return false;//returns 'false' if 's1','s2' not equual
                }
        ...
        */
        private static string ReversePrefix(string bulls_string6, char ch)
        {
            try
            {
                ...
                int i = 0, j = 0;
                //Take integer variable 'j' to get the index of
                for (i = 0; i < bulls_string6.Length; i++)
                {
                    if (ch == bulls_string6[i])// Checks whether character is there or not.
                    {
                        j = i;// Character is present means the index of the character will be assigned to j.
                    }
